...
export const getPodcastAnalytics = async (podcastId: string, podcastTitle?: string): Promise<PodcastAnalytics> => {
  ...
  if (!title) {
    ...
    title = podcast?.title || `Подкаст ${podcastId}`;
  }
...
export const getOverallAnalytics = async (): Promise<OverallAnalytics> => {
  const allRatings = await getAllRatings();
  const podcasts = await getAllPodcasts();
  const podcastAnalytics = await Promise.all(podcasts.map((p) => getPodcastAnalytics(p.id, p.title)));
